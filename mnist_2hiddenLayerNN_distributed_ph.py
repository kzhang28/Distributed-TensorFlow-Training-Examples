"""
    Distributed Tensorflow Example: two hidden layers neural network.
"""
...
tf.app.flags.DEFINE_integer("hidden_units",512,"the num of units in the hidden layer")
...
def main(_):
  ...
  if FLAGS.job_name == "ps":
      ...
      with tf.device(tf.train.replica_device_setter(
            ...
            cluster=cluster)):
          # Variable of the first hidden layer
          hid_w = tf.Variable(tf.truncated_normal([IMAGE_PIXELS * IMAGE_PIXELS, NUM_HIDDEN_1],
                              stddev=1.0 / IMAGE_PIXELS), name="hid_w")
          hid_b = tf.Variable(tf.zeros([NUM_HIDDEN_1]), name="hid_b")
          # Variable of the second hidden layer
          hid2_w = tf.Variable(tf.truncated_normal([NUM_HIDDEN_1, NUM_HIDDEN_2],
                                                   stddev=1.0/NUM_HIDDEN_2),name="hid_2_w")
          hid2_b = tf.Variable(tf.zeros([NUM_HIDDEN_2]),name="hid_2_b")
          # Variables of the softmax layer
          sm_w = tf.Variable(
              tf.truncated_normal([NUM_HIDDEN_2, 10],
                                  stddev=1.0 / math.sqrt(NUM_HIDDEN_1)),name="sm_w")
          ...
          hid_2_lin = tf.nn.xw_plus_b(hid,hid2_w,hid2_b)
          hid_2  = tf.nn.relu(hid_2_lin)
